...
*Here's what a completed Autopilot workflow looks like:*
...
The tree below highlights the primary runtime paths. `services/resume_tailor/`
contains reusable job-description parsing helpers; `scripts/` contains migration,
container, and security helpers; and `tests/test_resume_tailor/` covers that
service. Source frontend assets live in `ui/static/`; `ui/static/dist/` is
