...
SYSTEM_CONTEXT: str = build_llm_system_prompt(
    "Profession-neutral interview preparation coach",
    "Create practical interview questions and answer frameworks from the supplied facts.",
    extra_rules=(
        "Present questions and process details as possibilities unless the supplied company data confirms them.",
        "Use only actual candidate experiences in STAR frameworks; leave unsupported details explicit rather than completing them.",
        "Do not invent interview rounds, timelines, dress codes, salary ranges, achievements, or employer concerns.",
        "Treat total career years as domain-specific experience only when dated work history supports that claim.",
        "Focus on interview preparation without repeating the full upstream analyses.",
    ),
)
    ...
    async def generate(
        ...
        try:
            ...
            if response.get("filtered"):
                logger.warning(
                    "Interview prep response was filtered by safety settings"
                )
...
"""
        ...
        if candidate_risks:
            risks_list = "\n".join(
                [f"- {r.get('risk', 'N/A')}" for r in candidate_risks[:3]]
            )
            sections.append(
        ...
        return (
            "".join(sections)
            if sections
            else "No specific matching insights available."
        )
    ...
    def _create_filtered_result(self, message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "logistics": {
                ...
                "virtual_interview_tips": [],
                "post_interview": {
                    "thank_you_note": "Send within 24 hours",
